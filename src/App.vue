...
export default defineComponent({
  ...
  setup() {
    ...
    try {
      store = useStore();
      // ... rest of setup
      console.log("_STORE INIT: " + JSON.stringify(store));
    ...
    const checkWalletConnection = store.checkWalletConnection;

    const getMemoList = store.getMemoList;

    // Equivalent of your created lifecycle hook
    // TEMP OFF: state.c or 
    checkWalletConnection();

    // Equivalent of your watcher
    watch(getConnected, (newValue) => {
      if (newValue) {
        getMemoList();
      }
    });
